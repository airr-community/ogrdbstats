...
This section demonstrates how to conduct a V-gene analysis on a repertoire that has not been processed by a genotyping tool, and therefore for which no novel alleleshave been infered. 
...
This must match the germline set used to annotate the repertoire. Here we use the example reference set included with TIgGER. This R
...
- `v_germline_gapped.fasta` - The germline reference set used for annotation
...
Please note that the germline reference set must contain all germline sequences used for annotation. For V-gene analysis, the 
reference set must be gapped. The repertoire must be in AIRR or CHANGE-O format. 
