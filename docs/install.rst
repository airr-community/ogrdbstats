...
Once Pandoc is installed, please start the R interpreter and install ogrdbstats from CRAN. R >= 4.4 and Bioconductor >= 3.19 are required. For best results install ogrdbstats from Bioconductor, to handle dependencies automatically:

.. code-block:: R

    if (!requireNamespace("BiocManager", quietly = TRUE)) install.packages("BiocManager")
    BiocManager::install("ogrdbstats")
